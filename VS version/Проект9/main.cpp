...
char field[25][25] = {' '};                                          ///////////////////////////////////////////
field[player.element.x][player.element.y] = player.element.icon;     // player respawn
field[enemy1.element.x][enemy1.element.y] = enemy1.element.icon;     // enemy  respawn
for(int i = 0;i<candies;i++)                                         //
{                                                                    //
   int x = rand() % 24;                                              //
   int y = rand() % 24;                                              //  
   if (x==player.element.x&&y==player.element.y)                     //
   {                                                                 //
       i--;                                                          //
       continue;                                                     //          FIELD GENERATION
   }                                                                 //    
   field[x][y] = '*';                                                //
}                                                                    //
for (int i = 0;i<25;i++)                                             //
...
while (true)
{
    ...
    if (candies == counter.element.eaten)
    {
        ...
        cin >> answer;
        cout << endl;
        if (answer[0] == 'y'||answer[0] == 'Y'||answer[0] == 'Ä'||answer[0] == 'ä')
            gamebody();
        else
        {
